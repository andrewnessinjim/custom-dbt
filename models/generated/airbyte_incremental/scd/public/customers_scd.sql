{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_public.customers_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_public.customers_stg)"],
    tags = [ "top-level" ]
) }}
...
scd_data as (
    ...
    select
      ...
      lag(modified_date) over (
        partition by cast(customer_id as {{ dbt_utils.type_string() }})
        order by
